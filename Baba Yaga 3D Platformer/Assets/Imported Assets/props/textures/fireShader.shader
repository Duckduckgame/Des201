...
#include "UnityCG.cginc"
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		fixed4 noise = tex2D(_NoiseTex,fixed2((i.uv.x + _Time.x* _ScrollX) + uvdistort.b  ,(i.uv.y + _Time.x* _ScrollY))); //noise texture with distortion
...
#endif
		float4 flame = saturate(noise.g * _Hard); //noise flame
